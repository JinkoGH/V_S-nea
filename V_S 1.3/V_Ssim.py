...
class Sim():
    ...
    def sky_cycle(self):
        # timer sets at 0 from startup, and constantly updates when sky cycle is looped.
        now = pg.time.get_ticks()
        self.screen.fill(self.sky_colour)

        if 1000 < (now - self.timer):
            self.timer = now + 2000  # timer is shifted 2x to be ahead of now
            self.sky_colour = night
        if 1000 < (self.timer - now):
            self.timer = now
            self.sky_colour = day
        else:
            print("sky_cycle error")

        # Constantly checks the state of sky and updates the colour
